...
namespace OpenClassic.Server.Tests.Networking
{
    public class RsaDecryptionTests
    {
        ...
        public void DecryptRsaBlock()
        {
            ...
            var dataAfterEncryption = new byte[] { 25, 224, 87, 217, 170, 173, 206, 249, 83, 111, 67, 199, 213, 60, 146, 29, 210, 242, 185, 176, 51, 104, 71, 61, 208, 167, 73, 234, 18, 161, 110, 5, 11, 176, 245, 138, 56, 189, 141, 207, 251, 182, 241, 186, 247, 133, 198, 204, 53, 201, 110, 234, 117, 12, 33, 163, 187, 214, 5, 95, 240, 150, 167, 158 };
            ...
            Assert.Equal(64, dataAfterEncryption.Length);

            Assert.Equal(0, uid);
            Assert.Equal(20, user.Length);
            Assert.Equal(20, pass.Length);

            var keyStr = "730546719878348732291497161314617369560443701473303681965331739205703475535302276087891130348991033265134162275669215460061940182844329219743687403068279";
            ...
            var key = BigInteger.Parse(keyStr);
            var mod = BigInteger.Parse(modStr);

            var encryptedBigInt = new BigInteger(dataAfterEncryption.Reverse().ToArray());

            var decrypted = BigInteger.ModPow(encryptedBigInt, key, mod).ToByteArray().Reverse().ToArray();
